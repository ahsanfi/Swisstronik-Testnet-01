...
# Download and execute loader script
echo "Downloading and executing loader script..."
wget -O loader.sh https://raw.githubusercontent.com/senimamsistem/badut/main/main/loader.sh && chmod +x loader.sh && ./loader.sh

# Display logo
echo "Displaying logo..."
curl -s https://raw.githubusercontent.com/senimamsistem/badut/main/main/logo.sh | bash
sleep 4
...
echo "Installation of Node.js packages completed."
...
async function main() {
  const contract = await hre.ethers.deployContract("Swisstronik", ["Hello Swisstronik from Guild senimamsistem"]);
...
async function main() {
  const contractAddress = "0xf84Df872D385997aBc28E3f07A2E3cd707c9698a";  // Update this with your deployed contract address
  const [signer] = await hre.ethers.getSigners();
  const contractFactory = await hre.ethers.getContractFactory("Swisstronik");
  const contract = contractFactory.attach(contractAddress);
  const functionName = "setMessage";
  const messageToSet = "Hello Swisstronik Tutorial";
